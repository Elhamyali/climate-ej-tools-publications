...
# # Remove studies with "None" categories studies in the tool_use_type variable because we are not analyzing studies that Study unrelated to the tool (false positive in search) or not in English. Refer to codebook_studies in the 03_codebooks folder for more information on the category definitions

# Remove studies with "None" tool_use_type (per codebook)
...
all_studies_merged <- all_studies_merged %>%
  ...
  filter(!is.na(tool_use_type), str_trim(tool_use_type) != "None")
...
```{r descriptive-stat-9, echo = TRUE, message = FALSE, results = 'markup'}
#| label: descriptive-stat-9

# df_topics_tools <- all_studies_merged %>%
#   filter(publisher != "") %>%
#   count(publisher, sort = TRUE)
# 
# write_csv(df_all_publishers, here("06_viz_ready", "all_publishers.csv"))
...
```{r descriptive-stat-10, echo = TRUE, message = FALSE, results = 'markup'}
#| label: descriptive-stat-10

# df_tool_use_tools <- all_studies_merged %>%
#   filter(publisher != "") %>%
#   count(publisher, sort = TRUE)
# 
# write_csv(df_all_publishers, here("06_viz_ready", "all_publishers.csv"))
...
And voila! We're done. I'm going to take these csv that I exported in the `06_viz_ready` to further visualize outside of here. I'm a huge fan of highchartr and apache echart, but will most likely use Flourish for this one so my team at PEDP can update the viz should they pull new data in the future. It's also a great low code no code tool, great for beginners.
