...
A good resource that I'm following for IRA is *Cohen’s Kappa in R: For Two Categorical Variables* [@datanovia2025].
...
# --- Use only rows 1–44 and save to 05_cleaned_datasets ---
stopifnot(nrow(all_publishers_ira) >= 44)

ira_subset <- all_publishers_ira %>%
  slice(1:44) %>%
  filter(!is.na(pub_type_rater_1), !is.na(pub_type_rater_2))

out_dir <- here::here("05_cleaned_datasets")
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)
saveRDS(ira_subset, file.path(out_dir, "all_publishers_ira_rows1_44.rds"))

# --- Calculate Cohen’s Kappa on subset ---
tab_10x10 <- table(
  Rater1 = ira_subset$pub_type_rater_1,
  Rater2 = ira_subset$pub_type_rater_2
)
...
percent_agreement <- mean(ira_subset$pub_type_rater_1 == ira_subset$pub_type_rater_2)
percent_agreement
```
...
write_csv(tibble(total_studies = nrow(all_tools)), here("06_viz_ready", "total_studies.csv"))
```
